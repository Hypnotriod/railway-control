...
void TIMER0_Init(unsigned long microseconds, void (*pCallBack)())
{
	...
	if(cycles < TIMER0_RESOLUTION)				TCCR0 |= (1 << CS00);								// prescaller 1
	else if((cycles >>= 3) < TIMER0_RESOLUTION)	TCCR0 |= (1 << CS01);								// prescaller 8
	else if((cycles >>= 2) < TIMER0_RESOLUTION)	TCCR0 |= (1 << CS01) | (1 << CS00);					// prescaller 32
	else if((cycles >>= 1) < TIMER0_RESOLUTION)	TCCR0 |= (1 << CS02);								// prescaller 64
	else if((cycles >>= 1) < TIMER0_RESOLUTION)	TCCR0 |= (1 << CS02) | (1 << CS00);					// prescaller 128
	else if((cycles >>= 1) < TIMER0_RESOLUTION)	TCCR0 |= (1 << CS02) | (1 << CS01);					// prescaller 256
